...
# ---------------------------------------------------------
# 1. Robust Dataset with Error Handling
# ---------------------------------------------------------
class LanceBlobDataset(Dataset):
    def __init__(self, db_uri: str, table_name: str, transforms=None, id_col="id", blob_col="image", existing_ids=None):
        ...
        # Note: In a true streaming scenario with millions of rows, we might 
        # iterate differently, but this is the robust random-access method.
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # (Optimization: In a highly optimized setup, we would filter indices 
            #  in __init__, but checking here distributes the "skip" cost)
...
def main():
    ...
    if args.resume:
        ...
        # Efficiently scan only the ID column
        try:
